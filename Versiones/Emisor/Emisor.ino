...
void rfHandler()
{
  ...
  {                                                  //check if there is any rf message available
...
void pgHandler()
{
  ...
  {                      //if int pg was triggered
    ...
    else if ((index = pg.indexOf("MANVALV")) != -1)
    {                                       //if it is manual valve message interrupt
      ...
      { //for each oasis
        for (i = 0; i < TOTAL_OUTPUT; i++)
        { //go through each valve assigned
          if (sys.childValves[id][i] == v)
          {                                                                           //if its valve is equalt to v
    ...
    else if ((index = pg.indexOf("MANPRG")) != -1)
    {                                       //if it is a manual program message interrupt
    ...
    else if (pg.indexOf("MEMMORY") != -1)
    {                                                  //if it is memmory message interrupt
...
void executeCommand()
{
  ...
  case PLUG_PLAY:
    ...
    if (newChild(from, rfId))
    {                                          //check if it a new oasis
